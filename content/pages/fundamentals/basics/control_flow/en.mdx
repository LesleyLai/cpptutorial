...
so you just need to see the syntax.
...
auto main() -> int {
  ...
  if (x < 5) {
    ...
    std::puts("The condition is not met.");
  }
  std::puts("Good bye");
}
```
...
auto main() -> int {
  int x = 6;
  if (x % 2 == 0) {
    std::puts("x is divisible by 2");
  } else if (x % 3 == 0) {
    std::puts("x is divisible by 3");
  } else if (x % 5 == 0) {
    std::puts("x is divisible by 5");
  } else {
    std::puts("x is not divisible by 2, 3, or 5");
  }
}
```
...
so you should consider using it for real-world applications.
...
Thus, C++ and many other imperative languages
...
and the range-based for loop in the next section when we introduce containers.
...
For example,
the following function will print a number at least once
even if you pass a large number such as 100.

```cpp
auto print_numbers(int n) -> void {
  do {
    std::cout << n << '\n';
    ++n;
  } while (n < 10);
...
If you are performing an operation based on a count,
...
```cpp
auto print_numbers_no_5(int n) -> void {
for (int i = 0; i < 10; ++i) {
  if (i == 5) continue;
  std::cout << i << '\n';
}
...
We will talk about it extensively later.
